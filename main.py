import face_recognition
import numpy as np
import requests
import cv2
import io
import traceback
from fastapi import FastAPI, HTTPException
from pydantic import BaseModel
from typing import List

# --- QUALITY CONFIGURATION ---
# Reject photos darker than this (0-255)
MIN_BRIGHTNESS = 40   
# Reject photos blurrier than this (Higher = Sharper)
MIN_SHARPNESS = 60    

class FaceVerifyRequest(BaseModel):
    liveSelfieUrl: str
    profilePicUrl: str

class FaceVerifyResponse(BaseModel):
    isSamePerson: bool
    similarity: float
    distance: float
    warning: str = None

class EmbeddingRequest(BaseModel):
    imageUrl: str

class EmbeddingResponse(BaseModel):
    embedding: List[float]

app = FastAPI(title="Spark AI Server v2.0")

def load_image_from_url(url: str):
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        image_array = np.asarray(bytearray(response.content), dtype=np.uint8)
        img_bgr = cv2.imdecode(image_array, cv2.IMREAD_COLOR)
        if img_bgr is None: raise ValueError("Could not decode image.")
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        return img_rgb, img_bgr
    except Exception as e:
        print(f"Download Error: {e}")
        raise HTTPException(status_code=400, detail=f"Failed to load image: {str(e)}")

def check_quality(img_bgr):
    hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)
    brightness = np.mean(hsv[:, :, 2])
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    sharpness = cv2.Laplacian(gray, cv2.CV_64F).var()
    
    print(f"[Quality] Brightness: {brightness:.1f}, Sharpness: {sharpness:.1f}")
    
    if brightness < MIN_BRIGHTNESS:
        raise HTTPException(status_code=400, detail="Photo is too dark. Please find better lighting.")
    if sharpness < MIN_SHARPNESS:
        raise HTTPException(status_code=400, detail="Photo is too blurry. Please hold steady.")
    return True

@app.post("/verify-face", response_model=FaceVerifyResponse)
async def verify_face(request: FaceVerifyRequest):
    try:
        print(f"Processing 1:1 Verification...")
        img1_rgb, img1_bgr = load_image_from_url(request.liveSelfieUrl)
        img2_rgb, img2_bgr = load_image_from_url(request.profilePicUrl)
        
        check_quality(img1_bgr) # Strict check on the live selfie

        encodings1 = face_recognition.face_encodings(img1_rgb, num_jitters=1)
        encodings2 = face_recognition.face_encodings(img2_rgb, num_jitters=1)

        if not encodings1 or not encodings2:
            raise HTTPException(status_code=400, detail="No face detected. Please face the camera directly.")

        # Calculate Distance (Lower = Closer Match)
        face_distance = face_recognition.face_distance([encodings1[0]], encodings2[0])[0]
        
        # Calculate Similarity (Higher = Closer Match)
        similarity = max(0.0, 1.0 - face_distance)
        
        # Strict Threshold for 1:1 (Verification)
        is_same = face_distance < 0.55 

        return FaceVerifyResponse(isSamePerson=is_same, similarity=similarity, distance=face_distance)

    except HTTPException as e: raise e
    except Exception as e:
        print(f"Error: {traceback.format_exc()}") 
        raise HTTPException(status_code=500, detail=f"Server Error: {str(e)}")

@app.post("/generate-embedding", response_model=EmbeddingResponse)
async def generate_embedding(request: EmbeddingRequest):
    try:
        print(f"Generating Embedding...")
        img_rgb, img_bgr = load_image_from_url(request.imageUrl)
        check_quality(img_bgr) # Quality check before embedding
        
        encodings = face_recognition.face_encodings(img_rgb, num_jitters=1)
        if not encodings:
            raise HTTPException(status_code=400, detail="No face detected in the image.")

        return EmbeddingResponse(embedding=encodings[0].tolist())

    except HTTPException as e: raise e
    except Exception as e:
        print(f"Error: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Server Error: {str(e)}")

if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)
